...
npx prebuildify -t "$NODE_VERSION" -t "$NODE_VERSION2" -t "$NODE_VERSION3" -t "electron@$ELECTRON_VERSION" -t "electron@$ELECTRON_VERSION2" --napi=false --tag-uv --tag-armv --tag-libc --strip -n fsxattr
npx prebuildify -t "$NODE_VERSION" -t "$NODE_VERSION2" -t "$NODE_VERSION3" -t "electron@$ELECTRON_VERSION" -t "electron@$ELECTRON_VERSION2" --napi=true --tag-uv --tag-armv --tag-libc --strip -n fsxattr
